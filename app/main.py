...
def monitor_and_merge(job_id: str, total_chunks: int):
    time.sleep(30)  # Let splitter run
    set_total_chunks(job_id, total_chunks)
    set_job_status(job_id, {"status": "verifying", "total_chunks": total_chunks})
...
def get_status(job_id: str):
    ...
    data["processed_chunks"] = processed
    data["progress"] = f"{processed}/{total}" if total else "0/0"
